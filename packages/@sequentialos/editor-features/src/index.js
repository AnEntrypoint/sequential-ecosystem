export class EditorFeatures {
    ...
    document.addEventListener('keydown', (e) => {
      if (e.ctrlKey || e.metaKey) {
        if (e.key === 'k') {
          ...
          e.preventDefault();
          const templateCmd = this.editorType === 'flow' ? 'showFlowTemplateGallery' : 'showTemplateGallery';
          window[templateCmd]?.();
        }
      }
    });
  ...
  getCommands() {
    const baseCommands = [
      { id: 'run', name: `▶ Run ${this.getEditorLabel()}`, description: `Execute the ${this.getEditorLabel().toLowerCase()} (F5)` },
    ...
    return baseCommands.concat(this.extraCommands);
  }

  getEditorLabel() {
    const labels = { task: 'Task', tool: 'Tool', flow: 'Tool' };
    return labels[this.editorType] || 'Editor';
  }
  ...
  showKeyboardHelp() {
    const help = `
${this.getEditorLabel().toUpperCase()} EDITOR KEYBOARD SHORTCUTS
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Ctrl+K        Command Palette
Ctrl+F        Find in Code
Ctrl+G        Go to Line
Ctrl+/        Toggle Comment
${this.editorType === 'task' ? 'Ctrl+Shift+T  Template Gallery' : ''}
F5            Run ${this.getEditorLabel()}
Ctrl+S        Save
${this.editorType === 'task' ? 'Ctrl+Shift+S  Insert Snippet' : ''}
