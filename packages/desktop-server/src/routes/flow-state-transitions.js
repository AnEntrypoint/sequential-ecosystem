export class FlowStateTransitionValidator {
  ...
  validateStateTransitions(flow, statesArray) {
    ...
    statesArray.forEach(state => {
      this.validateState(state, statesArray, validation);
    });

    this.detectLoops(flow, statesArray, validation);
    this.detectTimeoutRaces(statesArray, validation);
    this.detectCompensationIssues(statesArray, validation);
    this.validateParallelJoins(statesArray, validation);
  ...
  detectUnreachableStates(flow, statesArray) {
    const stateMap = new Map(statesArray.map(s => [s.id, s]));
