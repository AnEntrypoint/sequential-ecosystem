...
Supabase-specific storage adaptor for sequential-runner with PostgreSQL backend.

## Architecture

sequential-adaptor-supabase provides:
...
Note: HTTP service implementations are in **sequential-wrapped-services** package. This adaptor only contains the Supabase storage backend.

## Storage Adapter

The SupabaseAdapter implements the base StorageAdapter interface from sequential-adaptor:
...
// Use with TaskExecutor from sequential-adaptor
...
Service implementations (deno-executor, task-executor, wrapped services) are in **sequential-wrapped-services** package:
...
Start wrapped services (from sequential-wrapped-services):
```bash
cd packages/sequential-wrapped-services
npx sequential --port 3100
```

### Production Deployment to Supabase

For Supabase cloud deployment, wrap the services from sequential-wrapped-services as edge functions:
...
Then deploy sequential-wrapped-services code to Supabase functions separately or use the deployment guide in sequential-wrapped-services/CLAUDE.md

## Service Implementations

All external service integrations are implemented in **sequential-wrapped-services**:

See `packages/sequential-wrapped-services/CLAUDE.md` for details on:
...
- **sequential-runner** - Core task execution engine (deployment-agnostic)
- **sequential-adaptor** - Base adaptor interface and execution logic
- **sequential-adaptor-sqlite** - SQLite adaptor for local development
- **sequential-wrapped-services** - Runtime-agnostic HTTP service implementations (deno-executor, wrapped services, etc.)
