export function createExecutionTrail() {
  ...
  return {
    ...
    addStep(executionId, stepName, stepType = 'execution', metadata = {}) {
      ...
      if (trail.steps.length > trailBufferSize) {
        trail.steps.shift();
      }
    ...
    getTrailSummary(executionId) {
      ...
      return {
        ...
        steps: trail.steps.map(function(s) { return s.name; }),
        duration: new Date(trail.startedAt) ? new Date() - new Date(trail.startedAt) : 0
      };
    },
